...
改めてになりますが、正直なところ、xmakeはOrdoに一番近い思想を持つツール、なんなら現段階では**上位互換**だと思っています。
...
大規模プロジェクトや複雑なビルドロジックが必要な場面では、CMakeの柔軟性に勝るものはありません。Ordoが狙っているのは、そうした複雑さが不要な新規プロジェクトの開発体験です。

### Mesonについて

書きやすいDSLとクリーンな設計は好きです。ただ、依存管理がWrapDBに限定されている点が、自分のユースケースでは不便でした。
